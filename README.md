...
| `TAVILY_PROXY_STATE_PATH`    |   否 | 操作系统标准状态目录          | 状态缓存文件                       |
...
- macOS：`~/Library/Application Support/tavily-proxy-mcp/state.json`
- Linux：`$XDG_STATE_HOME/tavily-proxy-mcp/state.json`，未设置时使用
  `~/.local/state/tavily-proxy-mcp/state.json`
- Windows：`%LOCALAPPDATA%\tavily-proxy-mcp\state.json`
